...
namespace Gaia
{
    ...
    public class GRCControllerSupport : GaiaRuntimeComponent
    {
        ...
        public float m_drawDistance = 250000;
        public GameObject m_customCharacter;
        public Camera m_customCamera;
#if HDPipeline
        public bool m_enableDynamicResolution;
        HDRenderPipelineAsset m_rpAsset;
#endif
        ...
        private GUIContent m_drawDistanceLabel;
        private GUIContent m_enableDynamicResolutionLabel;
        ...
        public override void Initialize()
        {
            ...
            if (m_drawDistanceLabel == null || m_drawDistanceLabel.text == "")
            {
                m_drawDistanceLabel = new GUIContent("Draw Distance", "The far clipping plane for the camera, or 'the render distance'.");
            }
            if (m_enableDynamicResolutionLabel == null || m_enableDynamicResolutionLabel.text == "")
            {
                m_enableDynamicResolutionLabel = new GUIContent("Dynamic Resolution", "Enables Dynamic Resolution and upscaling in the HDRP asset and on the camera if it is not enabled already.\r\n\r\nThis will render the game at lower resolution and upscale the result for a small performance boost.\r\n\r\nYou can look into native upscaling methods for Nvidia and ATI cards by setting up DLSS and FSR2 in the HD render pipeline asset. ");
            }
        ...
        public override void DrawUI()
        {
            ...
            EditorGUI.BeginChangeCheck();
            {
                ...
                m_drawDistance = EditorGUILayout.FloatField(m_drawDistanceLabel, m_drawDistance);
                DisplayHelp("Sets the far distance clipping plane of the camera to this value. Gaia sets up a high draw distance by default to render the sea far up to the horizon.");
                ...
                if (m_rpAsset == null)
                {
                    m_rpAsset = GaiaUtils.GetRenderPipelineAsset() as HDRenderPipelineAsset;
                }
                var dynRes = m_rpAsset.currentPlatformRenderPipelineSettings.dynamicResolutionSettings;
                if (dynRes.enabled)
                {
                    GUI.enabled = false;
                }
                m_enableDynamicResolution = EditorGUILayout.Toggle(m_enableDynamicResolutionLabel, m_enableDynamicResolution);
                if (dynRes.enabled)
                {
                    EditorGUILayout.LabelField("Dynamic Resolution enabled in the HDRP asset already.");
                }
        ...
        public override void AddToScene()
        {
            ...
            switch (m_selectedControllerType)
            {
                case ControllerType.FlyCam:
#if GAIA_2023_PRO
                    spawnedObject = GameObject.Instantiate(ControllerSettings.m_flyCamProPrefab);
#else
...
#if UNITY_EDITOR
            if (SceneView.lastActiveSceneView != null && SceneView.lastActiveSceneView.cameraSettings != null)
            {
                SceneView.lastActiveSceneView.cameraSettings.farClip = m_drawDistance;
            }
#endif
...
#if GAIA_V3_CINEMACHINE_OR_NEWER
                if (spawnedObject != null)
                {
                    Unity.Cinemachine.CinemachineCamera cVCam = spawnedObject.GetComponentInChildren<Unity.Cinemachine.CinemachineCamera>();
                    if (cVCam != null)
                    {
                        cVCam.Lens.FarClipPlane = 2000;
                    }
                }
#endif
            ...
            if (m_enableDynamicResolution)
            {
                ...
                var dynRes = m_rpAsset.currentPlatformRenderPipelineSettings.dynamicResolutionSettings;
                //we only perform actions on the hd render pipeline asset if dynamic res is not enabled (yet), otherwise we assume the user
                //has set up custom settings already
                if (!dynRes.enabled)
                {
                    var currentPlatformSettings = m_rpAsset.currentPlatformRenderPipelineSettings;

                    dynRes.enabled = true;
                    //same story with the upscalers, only mess with the configuration here if nothing has been set up already
                    if (dynRes.advancedUpscalersByPriority.Count == 0)
                    {
                        //STP should work without package install
                        dynRes.advancedUpscalersByPriority.Add(UnityEngine.Rendering.AdvancedUpscalers.STP);
                        dynRes.forceResolution = false;
                        dynRes.minPercentage = 50;
                        dynRes.maxPercentage = 100;
                        dynRes.dynResType = UnityEngine.Rendering.DynamicResolutionType.Hardware;
                        dynRes.upsampleFilter = UnityEngine.Rendering.DynamicResUpscaleFilter.EdgeAdaptiveScalingUpres;
                    }
                    currentPlatformSettings.dynamicResolutionSettings = dynRes;
                    m_rpAsset.currentPlatformRenderPipelineSettings = currentPlatformSettings;
#if UNITY_EDITOR
                    EditorUtility.SetDirty(m_rpAsset);
#endif
                ...
                if (hdacd != null)
                {
                    hdacd.allowDynamicResolution = true;
                }
            }
#endif
