...
#pragma warning restore 649
    ...
    public class Spawner : MonoBehaviour
    {
                ...
                if (m_spawnPreviewDirty == true)
                {
                    ...
                    for (int i = 0; i < m_previewRuleIds.Count; i++)
                    {
                        ...
                        if (m_settings.m_spawnerRules[m_previewRuleIds[i]].m_isActive)
                        {
                            ...
                            MultiTerrainOperationType mto = MultiTerrainOperationType.Heightmap;

                            if (m_settings.m_spawnerRules[m_previewRuleIds[i]].m_resourceType == SpawnerResourceType.TerrainTexture)
                            {
                                mto = MultiTerrainOperationType.Texture;
                            }

                            Graphics.Blit(ApplyBrush(operation, mto, m_previewRuleIds[i]), m_cachedPreviewColorRenderTextures[i]);
        ...
        private RenderTexture ApplyBrush(GaiaMultiTerrainOperation operation, MultiTerrainOperationType opType, int spawnRuleID = 0)
        {
            ...
            switch (opType)
            {
                ...
                case MultiTerrainOperationType.Texture:
                    if (operation.RTtextureSplatmap == null)
                    {
                        operation.GetSplatmap(null, true);
                    }
        ...
        public IEnumerator AreaSpawn(List<Spawner> spawners, BoundsDouble area, List<string> validTerrainNames = null)
        {
            ...
            for (Vector3 currentSpawnCenter = startPosition; Math.Round(currentSpawnCenter.x - spawnRange,2) < Math.Round(area.max.x,2); currentSpawnCenter += new Vector3(locationIncrement, 0f, 0f))
            {
                if (!m_cancelSpawn)
                {
                ...
                //We do not want to overshoot with the spawner - move it so it aligns with the borders of the spawn area if necessary
                if (currentSpawnCenter.x > area.max.x)
                {
                    currentSpawnCenter = new Vector3((float)area.max.x - spawnRange, currentSpawnCenter.y, currentSpawnCenter.z);
                }

                //Iterating across the area - Z Axis
                for (currentSpawnCenter = new Vector3(currentSpawnCenter.x, currentSpawnCenter.y, startPosition.z); Math.Round(currentSpawnCenter.z - spawnRange, 2) < Math.Round(area.max.z, 2); currentSpawnCenter += new Vector3(0f, 0f, locationIncrement))
                {

                    //We do not want to overshoot with the spawner - move it so it aligns with the borders of the spawn area if necessary
                    if (currentSpawnCenter.z > area.max.z)
                    {
                        currentSpawnCenter = new Vector3(currentSpawnCenter.x, currentSpawnCenter.y, (float)area.max.z - spawnRange);
                    }
                    ...
                    //We need to load terrains in before being able to remove anything - but we only should do so if the 
                    //world creation is not running at the same time, this can lead to problems otherwise
                    if (GaiaUtils.HasDynamicLoadedTerrains() && !SessionManager.m_worldCreationRunning)
                    {
        ...
        private void ApplyNoiseSettingsToImageMask(ImageMask imageMask, BaseTerrainNoiseSettings btns, float sizemultiplicator)
        {
            ...
            //apply a multiplicator to billow noise, since it handles this
            //setting quite differently than the other noise types
            ...
            fbm.lacunarity = multiplicator * btns.m_shapeGranularity;
                    ...
                    if (m_settings.m_spawnerRules[i].m_isActive)
                    {
                        switch (m_settings.m_spawnerRules[i].m_resourceType)
                        {
                            ...
                            case GaiaConstants.SpawnerResourceType.Probe:
                                ...
                                PWS_WaterSystem gaiawater = GameObject.FindAnyObjectByType<PWS_WaterSystem>();
                                if (gaiawater != null)
                                {
        ...
        public void SpawnStamps()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < outputStampPositions.Length; i++)
                {
                    ...
                    //First check in the "regular" stamp directory
                    string directory = GaiaStampDirectory + Path.DirectorySeparatorChar + selectedStampPrototype.m_featureType;
                    var info = new DirectoryInfo(directory);
                    if (!Directory.Exists(directory))
                    {
                        //Then check in the "user data" stamp directory
                        directory = GaiaUserStampDirectory + Path.DirectorySeparatorChar + selectedStampPrototype.m_featureType;
                        info = new DirectoryInfo(directory);
                        if (!Directory.Exists(directory))
                        {
                            //Then check in the legacy stamp directory
                            directory = GaiaLegacyStampDirectory + Path.DirectorySeparatorChar + selectedStampPrototype.m_featureType;
                            info = new DirectoryInfo(directory);
                        }

                        if (!Directory.Exists(directory))
                        {
                            Debug.LogError($"Could not find the stamp category '{selectedStampPrototype.m_featureType}' in any stamp directory! This category will be skipped for the generation.");
                            continue;
                        }
        ...
        public void AddToSession(GaiaOperation.OperationType opType, string opName)
        {
            ...
            if (SessionManager != null && SessionManager.IsLocked() != true && SessionManager.m_recordSession)
            {
        ...
        public void LoadSettings(SpawnerSettings settingsToLoad, bool createDetailSOs = true)
        {
            m_settings.ClearImageMaskTextures();
