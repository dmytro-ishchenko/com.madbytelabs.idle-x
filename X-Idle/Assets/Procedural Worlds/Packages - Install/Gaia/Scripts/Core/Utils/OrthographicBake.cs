...
namespace Gaia
{
    ...
    public class OrthographicBake
    {
        ...
        private static List<Volume> m_deactivatedVolumes = new List<Volume>();
#endif
        ...
        public static void LightsOff()
        {
            ...
            foreach (Volume volume in allVolumes)
            {
                if (volume.isActiveAndEnabled)
                {
                    ...
                    m_deactivatedVolumes.Add(volume);
                }
            }
#endif
...
#if HDPipeline || UPPipeline
            foreach (Volume volume in m_deactivatedVolumes)
            {
                if (volume != null)
                {
                    volume.enabled = true;
