...
namespace Gaia
{
    ...
    public class TerrainHelper : MonoBehaviour
    {
        ...
        public static void SetTerrainMaterial(Material material, string title="", string message ="", string OK="", string cancel="", bool onlyStandardShader = true)
        {
            ...
            if (GaiaUtils.HasTerrains())
            {
                if (title == "" || GaiaUtils.DisplayDialogNoEditor("Set up Terrain Material?", message, "Continue", "Cancel"))
                {
                    ...
                    {
                        Action<Terrain> act = (terrain) => { SwapMaterial(terrain, material, onlyStandardShader); };
                    ...
                    else
                    {
                        ...
                        {
                            SwapMaterial(terrain, material, onlyStandardShader);
                        }
                    }
                }
            }
        ...
        public static void SwapMaterial(Terrain t, Material targetMaterial, bool onlyStandardShaders)
        {
            string[] standardShaderNames = new string[3] { "HDRP/TerrainLit", "URP/TerrainLit", "Nature/Terrain/Standard" };

            if (!onlyStandardShaders || standardShaderNames.Contains(t.materialTemplate.shader.name))
            {
                t.materialTemplate = targetMaterial;
            }
        ...
        /// <summary>
        /// Get the terrain that matches this location (OR NEAREST TERRAIN), otherwise return null
        /// </summary>
        /// <param name="locationWU">Location to check in world units</param>
        /// <returns>Terrain here or null</returns>
        ...
        /// <summary>
        /// Get the terrain that strictly matches this location, otherwise return null
        /// </summary>
        /// <param name="locationWU">Location to check in world units</param>
        /// <returns>Terrain here or null</returns>
        public static Terrain GetTerrainStrict(Vector3 locationWU, bool selectWorldMapTerrains = false)
        {
            Terrain terrain;
            Vector3 terrainMin = new Vector3();
            Vector3 terrainMax = new Vector3();
        ...
        {
            // Unity terrain pivot is at the -x -z corner, so we calculate bounds directly from position and size
            Vector3 terrainPosition = t.GetPosition();
            Vector3 terrainSize = t.terrainData.size;
            
            // Calculate the center and create bounds
            Vector3 center = terrainPosition + terrainSize * 0.5f;
            Bounds worldSpaceBounds = new Bounds(center, terrainSize);
        ...
        public static void ResizeHeightmap(Terrain terrain, int targetResolution)
        {
            ...
            RenderTexture oldHoles = RenderTexture.GetTemporary(terrain.terrainData.holesTexture.width, terrain.terrainData.holesTexture.height, 0, GraphicsFormat.R8_UNorm);
            ...
            RenderTexture newHoles = RenderTexture.GetTemporary(terrain.terrainData.holesTexture.width, terrain.terrainData.holesTexture.height, 0, GraphicsFormat.R8_UNorm);
        ...
        public static Terrain GetTerrainNeighbor(Terrain terrain, StitchDirection direction)
        {
            ...
            {
                if (t == terrain)
                    continue; // Skip the terrain itself
                    
                Bounds boundsTerrain = GetWorldSpaceBounds(t);
                const float adjacencyTolerance = 1.5f; // Tolerance for checking if terrains are adjacent
                
                // Check if terrains are adjacent (touching) - Intersects returns true for touching bounds
                // Also check if they're very close (within tolerance) in case of floating point precision issues
                bool isAdjacent = boundsTerrain.Intersects(boundsOriginTerrain) || 
                                  (Mathf.Abs(boundsOriginTerrain.min.x - boundsTerrain.max.x) < adjacencyTolerance && 
                                   boundsTerrain.max.z > boundsOriginTerrain.min.z && boundsTerrain.min.z < boundsOriginTerrain.max.z) ||
                                  (Mathf.Abs(boundsOriginTerrain.max.x - boundsTerrain.min.x) < adjacencyTolerance && 
                                   boundsTerrain.max.z > boundsOriginTerrain.min.z && boundsTerrain.min.z < boundsOriginTerrain.max.z) ||
                                  (Mathf.Abs(boundsOriginTerrain.min.z - boundsTerrain.max.z) < adjacencyTolerance && 
                                   boundsTerrain.max.x > boundsOriginTerrain.min.x && boundsTerrain.min.x < boundsOriginTerrain.max.x) ||
                                  (Mathf.Abs(boundsOriginTerrain.max.z - boundsTerrain.min.z) < adjacencyTolerance && 
                                   boundsTerrain.max.x > boundsOriginTerrain.min.x && boundsTerrain.min.x < boundsOriginTerrain.max.x);
                
                if (isAdjacent)
                {
                    const float edgeTolerance = 1.5f; // Tolerance for edge matching (slightly larger than 1 unit to handle floating point precision)
                    
                    switch (direction)
                    {
                        case StitchDirection.North:
                            if (Mathf.Abs(boundsOriginTerrain.max.z - boundsTerrain.min.z) < edgeTolerance && boundsTerrain.max.x > boundsOriginTerrain.min.x && boundsTerrain.min.x < boundsOriginTerrain.max.x)
                            {
                                neighbor = t;
                            }
                            break;
                        case StitchDirection.South:
                            if (Mathf.Abs(boundsOriginTerrain.min.z - boundsTerrain.max.z) < edgeTolerance && boundsTerrain.max.x > boundsOriginTerrain.min.x && boundsTerrain.min.x < boundsOriginTerrain.max.x)
                            {
                                neighbor = t;
                            }
                            break;
                        case StitchDirection.East:
                            if (Mathf.Abs(boundsOriginTerrain.max.x - boundsTerrain.min.x) < edgeTolerance && boundsTerrain.max.z > boundsOriginTerrain.min.z && boundsTerrain.min.z < boundsOriginTerrain.max.z)
                            {
                                neighbor = t;
                            }
                            break;
                        case StitchDirection.West:
                            if (Mathf.Abs(boundsOriginTerrain.min.x - boundsTerrain.max.x) < edgeTolerance && boundsTerrain.max.z > boundsOriginTerrain.min.z && boundsTerrain.min.z < boundsOriginTerrain.max.z)
                            {
                                neighbor = t;
                            }
                            break;
