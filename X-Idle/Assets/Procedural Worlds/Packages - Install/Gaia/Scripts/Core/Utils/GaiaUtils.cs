...
namespace Gaia
{
    ...
    public enum GaiaPackageVersion { Unity2019_1, Unity2019_2, Unity2019_3, Unity2019_4, Unity2020_1, Unity2020_2, Unity2020_3, Unity2020_4, Unity2021_1, Unity2021_2, Unity2021_3, Unity2021_4, Unity2022_1, Unity2022_2, Unity2022_3, Unity2022_4, Unity2023_1, Unity2023_2, Unity6000_0, Unity6000_1, Unity6000_2}
...
#region Asset directory helpers
        ...
        public static void RefreshPlayerSetup()
        {
            Camera camera = GetCamera();
            if (camera != null)
            {
                GaiaGlobal.FinalizeCameraObjectRuntime(camera, true);
            }

            GameObject playerGO = GetPlayerGameObject();
            if (playerGO != null)
            {
                GaiaGlobal.FinalizePlayerObjectRuntime(playerGO, true);
            }
        }
        ...
        public static void RefreshPipeline(GaiaSettings gaiaSettings = null)
        {
            if (gaiaSettings == null)
            {
                gaiaSettings = GetGaiaSettings();
            }
            ...
            else if (GaiaUtils.GetRenderPipelineAsset().GetType().ToString().Contains("HDRenderPipelineAsset"))
            {
#if HDPipeline
                gaiaSettings.m_pipelineProfile.m_activePipelineInstalled = GaiaConstants.EnvironmentRenderer.HighDefinition;
                gaiaSettings.m_currentRenderer = GaiaConstants.EnvironmentRenderer.HighDefinition;
#else
                gaiaSettings.m_currentRenderer = GaiaConstants.EnvironmentRenderer.BuiltIn;
                gaiaSettings.m_pipelineProfile.m_activePipelineInstalled = GaiaConstants.EnvironmentRenderer.BuiltIn;
#endif
            }
            else if (GaiaUtils.GetRenderPipelineAsset().GetType().ToString().Contains("UniversalRenderPipelineAsset"))
            {
#if UPPipeline
                gaiaSettings.m_currentRenderer = GaiaConstants.EnvironmentRenderer.Universal;
                gaiaSettings.m_pipelineProfile.m_activePipelineInstalled = GaiaConstants.EnvironmentRenderer.Universal;
#else
                gaiaSettings.m_currentRenderer = GaiaConstants.EnvironmentRenderer.BuiltIn;
                gaiaSettings.m_pipelineProfile.m_activePipelineInstalled = GaiaConstants.EnvironmentRenderer.BuiltIn;
#endif
            ...
            AssetDatabase.SaveAssetIfDirty(AssetDatabase.GUIDFromAssetPath(AssetDatabase.GetAssetPath(gaiaSettings)));
            if (gaiaSettings.m_pipelineProfile != null)
            {
                EditorUtility.SetDirty(gaiaSettings.m_pipelineProfile);
                AssetDatabase.SaveAssetIfDirty(AssetDatabase.GUIDFromAssetPath(AssetDatabase.GetAssetPath(gaiaSettings.m_pipelineProfile)));
            }
#endif
        }
...
#if UNITY_EDITOR
        [MenuItem(GaiaConstants.gaiaPolyMaskMenuEntry)]
        public static void CreatePolyMaskMenu()
        { 
            PolyMask newMask = CreatePolyMask();
            EditorGUIUtility.PingObject(newMask);
            Selection.activeObject = newMask.gameObject;
        }
#endif


        /// <summary>
        /// Creates a new polygon mask object
        /// </summary>
        /// <returns></returns>

        public static PolyMask CreatePolyMask()
        {
            GameObject polyMaskContainerGO = GaiaUtils.GetPolyMaskGameObject();

            string[] allNames = new string[polyMaskContainerGO.transform.childCount];
            for (int i = 0; i < polyMaskContainerGO.transform.childCount; i++)
            {
                allNames[i] = polyMaskContainerGO.transform.GetChild(i).name;  
            }
#if UNITY_EDITOR
            string newName = ObjectNames.GetUniqueName(allNames, "New Poly Mask");
#else
            string newName = "New Poly Mask";
#endif

            GameObject newPolyMaskGO = new GameObject(newName);
            newPolyMaskGO.tag = "EditorOnly";
            newPolyMaskGO.hideFlags = HideFlags.DontSaveInBuild;
            newPolyMaskGO.transform.parent = polyMaskContainerGO.transform;
            PolyMask polyMask = newPolyMaskGO.AddComponent<PolyMask>();
            polyMask.MaskType = PolyMask.PolyMaskType.Closed;
            polyMask.Initialize();
            return polyMask;
        }

        public static GameObject GetPolyMaskGameObject()
        {
            GameObject gaiaTools = GetGaiaGameObject();
            Transform t = gaiaTools.transform.Find(GaiaConstants.gaiaPolyMaskObject);
            GameObject polyMasks = null;
            if (t != null)
            {
                 polyMasks = t.gameObject;
            }
            if (polyMasks == null)
            {
...
#region Asset, Scriptable Object, GameObject helpers
        ...
        public static bool ProcessMaterialLibrary(List<Material> materialsToProcess = null)
        {
            ...
            if (gaiaSettings == null || gaiaSettings.m_pipelineProfile == null)
            {
                return false;
            }
        ...
        public static void CheckHDRPLightRenderMode(Light light,HDAdditionalLightData lightData, float currentLightIntensity)
        {
            if (lightData != null)
            {
                if (GaiaHDRPRuntimeUtils.GetLightUnit(light, lightData) != LightUnit.Lux)
                {
                    GaiaHDRPRuntimeUtils.SetLightUnit(light, lightData, LightUnit.Lux);
                    GaiaHDRPRuntimeUtils.SetLightIntensity(light, lightData, currentLightIntensity);
                }
            }
        }
        ...
        public static GameObject GetCharacter()
        {
            ...
            CharacterController characterController = GameObject.FindAnyObjectByType<CharacterController>();
        ...
        public static bool CheckIfPWSkyExists()
        {
#if GAIA_2023_PRO
            if (PWSkyStandalone.Instance != null)
            {
                return true;
            }
            else
            ...
            try
            {
                ...
                if (SerializedObjectTagManager != null)
                {
                    ...
                    if (serializedProperties != null)
                    {
                        ...
                        {
                            //skipping problematic prefabs
                            if (paths[i].Contains("Water Surface"))
                            {
                                continue;
                            }

                            if (paths[i].Contains("FlyCam"))
                            {
                                continue;
                            }

                            if (paths[i].Contains("Gaia Weather"))
                            {
                                continue;
                            }
...
#region Programming helpers
        ...
        /// <summary>
        /// Gets a List of types for all classes that inherit from a certain source class type. Works only in the editor.
        /// </summary>
        ...
        /// <summary>
        /// Safely initializes wind shader globals using reflection.
        /// If WindControl class or method doesn't exist, fails gracefully without errors.
        /// </summary>
        /// <param name="logResult">Whether to log success/failure to console</param>
        /// <returns>True if initialization succeeded, false otherwise</returns>
        public static bool TryInitializeWindShaderGlobals(bool logResult = true)
        {
            try
            {
                /// <summary>
                /// Initializes wind shader globals with zero/neutral values to ensure vegetation renders correctly without wind animation.
                /// Can be called manually to initialize wind parameters before any WindControl instance exists in the scene.
                /// </summary>

                // Initialize with zero wind strength but valid data structures
                // Wind direction data (neutral forward direction, time = 0)
                var _WindData_0_0 = new Vector4(0f, 0f, 1f, 1f); // Forward direction with enabled flag
                var _WindData_0_1 = new Vector4(0f, 0f, 1f, 0f); // Stable direction with time = 0
                var _WindData_0 = new Matrix4x4(_WindData_0_0, _WindData_0_1, Vector4.zero, Vector4.zero);
                Shader.SetGlobalMatrix("_WindData_0", _WindData_0.transpose);

                // Grass wind parameters (all zeroed)
                var _WindData_1_0 = new Vector4(0f, 0f, 0f, 0f); // Base strength and phases = 0
                var _WindData_1_1 = new Vector4(1f, 0f, 0f, 0f); // Variance period = 1 to avoid division by zero, rest = 0
                var _WindData_1_2 = new Vector4(0f, 1f, 0f, 0f); // Variance period = 1, rest = 0
                var _WindData_1_3 = new Vector4(0f, 0f, 0f, 1f); // Flutter variance period = 1 to avoid division by zero
                var _WindData_1 = new Matrix4x4(_WindData_1_0, _WindData_1_1, _WindData_1_2, _WindData_1_3);
                Shader.SetGlobalMatrix("_WindData_1", _WindData_1.transpose);

                // Tree wind parameters (all zeroed)
                var _WindData_2_0 = new Vector4(0f, 0f, 0f, 0f); // Base strength and phases = 0
                var _WindData_2_1 = new Vector4(1f, 0f, 0f, 0f); // Variance period = 1, rest = 0
                var _WindData_2_2 = new Vector4(0f, 1f, 0f, 0f); // Variance period = 1, rest = 0
                var _WindData_2_3 = new Vector4(0f, 0f, 0f, 1f); // Flutter variance period = 1
                var _WindData_2 = new Matrix4x4(_WindData_2_0, _WindData_2_1, _WindData_2_2, _WindData_2_3);
                Shader.SetGlobalMatrix("_WindData_2", _WindData_2.transpose);

                Shader.SetGlobalFloat("_WindEnabled", 0f);
                return true;
            }
            catch (Exception ex)
            {
                if (logResult)
                    Debug.LogWarning($"Failed to initialize wind shader globals: {ex.Message}");
        ...
        public static float GetTerrainDetailShaderDistance()
        {
            return Shader.GetGlobalVector("_PW_DetailRenderDistance").x; 
        }

        public static float GetTerrainDetailFadeOutDistance()
        {
            return Shader.GetGlobalVector("_PW_DetailRenderDistance").y;
        }


        public static void SetTerrainDetailShaderDistances(float detailDrawDistance, float detailFadeOutDistance)
        {
            Shader.SetGlobalVector("_PW_DetailRenderDistance", new Vector4(Mathf.Max(0f,detailDrawDistance), Mathf.Max(0f,detailFadeOutDistance), 0, 0));
        }

        /// <summary>
        /// Sets up a player character with the required components so it works correctly with all
        /// associated Gaia systems
        /// </summary>
        /// <param name="character"></param>
        public static void SetUpGaiaCharacter(GameObject character)
        {

            GaiaSettings gaiaSettings = GetGaiaSettings();

#if GAIA_2023_PRO
            //Terrain loading - the character needs to have a terrain loader script
            //& the "wait for terrain loading" script if a rigidbody is present
            ...
            if (dynamicLoadedTerrains)
            {
                ...
                TerrainLoader loader = character.GetComponent<TerrainLoader>();
                if (loader == null)
                {
                    loader = character.AddComponent<TerrainLoader>();
                    ...
                    loader.m_loadingBoundsRegular = new BoundsDouble(character.transform.position, new Vector3(size, size, size));
                    loader.m_loadingBoundsImpostor = new BoundsDouble(character.transform.position, new Vector3(size * 3f, size * 3f, size * 3f));
                }

                //Add the wait script for all rigidbodies that use gravity
                foreach (Rigidbody rigidbody in character.GetComponentsInChildren<Rigidbody>())
                {
            ...
            {
                FloatingPointFix fix = character.GetComponent<FloatingPointFix>();
                if (fix == null)
                {
                    fix = character.AddComponent<FloatingPointFix>();
                }
        ...
        public static void SetupGaiaCamera(Camera camera)
        {
            //Add simple layer culling script
            SimpleCameraLayerCulling layerCulling = camera.GetComponent<SimpleCameraLayerCulling>();
            if (layerCulling == null)
            {
                layerCulling = camera.gameObject.AddComponent<SimpleCameraLayerCulling>();
            }
            layerCulling.m_applyToGameCamera = true;
            layerCulling.m_applyToSceneCamera = true;
            layerCulling.Initialize();
#if UNITY_EDITOR
            foreach (var sceneCamera in SceneView.GetAllSceneCameras())
            {
                sceneCamera.layerCullDistances = layerCulling.m_profile.m_layerDistances;
            }
#endif
            layerCulling.ApplyToGameCamera();
        ...
        /// <summary>
        /// Check to see it a point is on screen
        /// </summary»
        public static bool IsOnScreen(Vector3 position, float maxDistance = float.MaxValue)
        {
        ...
        public static GameObject GetGaiaGameObject()
        {
            ...
            if (gaiaObj == null)
            {
                ...
                gaiaObj.AddComponent<GaiaSceneReadMe>();
            }
            return gaiaObj;
        }

        public static GameObject GetWeatherObject(bool create = true)
        {
            return GetRuntimeChild(GaiaConstants.gaiaWeatherObject, create);
        }

        public static GameObject GetLightingObject(bool create = true)
        {
            return GetRuntimeChild(GaiaConstants.gaiaLightingObject, create);
        }

        public static GameObject GetPWSkyObject(bool create = true)
        {
            return GetRuntimeChild(GaiaConstants.pwSkyObject, create);
        }

        public static GameObject GetWaterObject(bool create = true)
        {
            return GetRuntimeChild(GaiaConstants.gaiaWaterObject, create);
        }

        public static GameObject GetRuntimeChild(string childName, bool create)
        {
            GameObject childObj = GameObject.Find(childName);
            if (childObj == null && create)
            {
                GameObject runtimeObject = GetRuntimeSceneObject(true);
                childObj = new GameObject(childName);
                childObj.transform.parent = runtimeObject.transform;
            }
            return childObj;
        }
        ...
        {
            GameObject playerObject = GameObject.Find("FlyCam");
            if (playerObject != null)
            {
                return playerObject;
            }

            GameObject playerParentGO = GetPlayerObject(false);
            if (playerParentGO != null)
            {
                CharacterController cc = playerParentGO.GetComponentInChildren<CharacterController>();
                if (cc != null)
                {
        ...
        public static void SetRenderPipelineAsset(RenderPipelineAsset rpAsset)
        {
#if UNITY_6000_0_OR_NEWER
        ...
        public static bool UsesCorrectPipelineDefines()
        {
            ...
            {
                if (GaiaUtils.GetRenderPipelineAsset().GetType().ToString().Contains("HDRenderPipelineAsset"))
                {
#if HDPipeline && !UPPipeline
                    return true;
#else
        ...
        public static void ResetBiomePresets(bool forceReset=false)
        {
            ...
            {
                string[] allSpawnerPresetGUIDs = AssetDatabase.FindAssets("t:BiomePreset", new string[1] { GaiaDirectories.GetPackageInstallDirectory() });
                ...
                string[] allSpawnerGUIDs = AssetDatabase.FindAssets("t:SpawnerSettings l:" + GaiaConstants.gaiaManagerSpawnerLabel, new string[1] { GaiaDirectories.GetPackageInstallDirectory() });
                ...
                string[] allTerrainExportGUIDS = AssetDatabase.FindAssets("t:ExportTerrainSettings", new string[1] { GaiaDirectories.GetPackageInstallDirectory() });
        ...
        public static bool DoesTagExist(string v)
        {
            bool success;
            try
            {
                GameObject.FindGameObjectsWithTag(v);
                success =  true;
            }
            catch(Exception ex)
            {
                if (ex.Message == "")
                { 
                    //Just to avoid compiler warning about unused "ex"
                }
                success = false;    
            }
            return success;
        ...
        public static GaiaPackageVersion GetPackageVersion()
        {
            ...
            else if (Application.unityVersion.Contains("2023.2"))
            {
                return GaiaPackageVersion.Unity2023_2;
            }
            else if (Application.unityVersion.Contains("6000.0"))
            {
                return GaiaPackageVersion.Unity6000_0;
            }
            else if (Application.unityVersion.Contains("6000.1"))
            {
                return GaiaPackageVersion.Unity6000_1;
            }
            else if (Application.unityVersion.Contains("6000.2"))
            {
                return GaiaPackageVersion.Unity6000_2;
            }
        ...
        /// <summary>
        /// Checks if a given spawn rule is valid for the current render pipeline
        /// </summary>
        /// <param name="sr"></param>
        /// <returns></returns>
        /// <exception cref="NotImplementedException"></exception>
        public static bool IsValidSpawnRuleforRP(SpawnRule sr)
        {
            EnvironmentRenderer er = GetActivePipeline();

            if (er == EnvironmentRenderer.BuiltIn && sr.m_RP_BuiltIn ||
                er == EnvironmentRenderer.Universal && sr.m_RP_URP ||
                er == EnvironmentRenderer.HighDefinition && sr.m_RP_HDRP)
            {
                return true;
            }
            else
