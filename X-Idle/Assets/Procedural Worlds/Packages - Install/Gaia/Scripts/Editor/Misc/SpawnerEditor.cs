...
namespace Gaia
{
    ...
    public class SpawnerEditor : PWEditor, IPWEditor
    {
        ...
        private string m_gaiaStampDirectory = "";
        private string m_gaiaUserStampDirectory = "";
        private string m_gaiaLegacyStampDirectory = "";

        private string GaiaStampDirectory
        {
            get
            {
                if (string.IsNullOrEmpty(m_gaiaStampDirectory))
                {
                    m_gaiaStampDirectory = GaiaDirectories.GetStampDirectory();
                }
                return m_gaiaStampDirectory;
            }
        }

        private string GaiaUserStampDirectory
        {
            get
            {
                if (string.IsNullOrEmpty(m_gaiaUserStampDirectory))
                {
                    m_gaiaUserStampDirectory = GaiaDirectories.GetUserStampDirectory();
                }
                return m_gaiaUserStampDirectory;
            }
        }

        private string GaiaLegacyStampDirectory
        {
            get
            {
                if (string.IsNullOrEmpty(m_gaiaLegacyStampDirectory))
                {
                    m_gaiaLegacyStampDirectory = GaiaDirectories.GetLegacyStampDirectory();
                }
                return m_gaiaLegacyStampDirectory;
            }
        }
        ...
        void OnEnable()
        {
            ...
            //Fill the enum popup for stamp categories / feature names from the stamp directories
            m_stampCategoryNames.Clear();

            var info = new DirectoryInfo(GaiaStampDirectory);
            ...
            if (Directory.Exists(GaiaUserStampDirectory))
            {
                info = new DirectoryInfo(GaiaUserStampDirectory);
            ...
            //Do the same with the legacy stamp folder
            if (Directory.Exists(GaiaLegacyStampDirectory))
            {
                info = new DirectoryInfo(GaiaLegacyStampDirectory);
                fileInfo = info.GetDirectories();
                foreach (DirectoryInfo dir in fileInfo)
                {
                    m_stampCategoryNames.Add(dir.Name);
            ...
            if (m_spawner.m_worldSizeAdvancedUnfolded)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    EditorGUILayout.BeginVertical();
                    {
                        ...
                        wcs.m_addLoadingScreen = m_editorUtils.Toggle("AddLoadingScreen", wcs.m_addLoadingScreen, helpEnabled);
        ...
        private void StartExport()
        {
            ...
            try
            {
                ...
                if (m_spawner.m_worldMapStamperSettings.Count() == 0 && !EditorUtility.DisplayDialog("Create new World without Stamps", "WARNING: You are about to generate terrains, but you did not spawn any stamps. This will result in very 'basic' terrains without distinct features. Normally you would want to spawn in stamps first with the 'Randomize Stamps' button. Do you want to continue the generation regardless?", "Continue", "Cancel"))
                {
                    return;
                }
                ...
                GaiaSessionManager.OnWorldCreated -= OnWorldMapStampingFinished;
                GaiaSessionManager.OnWorldCreated += OnWorldMapStampingFinished;
                if (GaiaSessionManager.CreateOrUpdateWorld(m_spawner.m_worldCreationSettings, true))
                {
                    m_spawner.m_ExportRunning = true;
                }
        ...
        private void OnWorldMapStampingFinished()
        {
            ...
            finally
            {
                ...
                else
                {
                    ...
                    bc = FindAnyObjectByType<BiomeController>();
                    if (bc != null)
                    {
                        Selection.activeObject = bc.gameObject;
                    }
        ...
        private bool DrawSingleRulePanel(SpawnRule rule, int ruleIdx)
        {
            ...
            string pipelineInfo = "";
            //only display pipeline info if the rule is actually pipeline specific
            if (!(rule.m_RP_BuiltIn && rule.m_RP_HDRP && rule.m_RP_URP))
            {
                pipelineInfo = " [";
                if (rule.m_RP_BuiltIn || rule.m_RP_HDRP || rule.m_RP_URP)
                {
                   
                    if (rule.m_RP_BuiltIn)
                    {
                        pipelineInfo += "Built-In";
                    }
                    if (rule.m_RP_URP)
                    {
                        if (rule.m_RP_BuiltIn)
                    ...
                    if (rule.m_RP_HDRP)
                    {
                        if (rule.m_RP_BuiltIn || rule.m_RP_URP)
                        {
                            pipelineInfo += ",";
                        }
                        pipelineInfo += "HDRP";
                ...
                {
                    pipelineInfo += "No active RP";
                }
                pipelineInfo += "]";
            }

            GUIContent panelLabel = new GUIContent(instanceCount + " " + rule.m_name + pipelineInfo);
        ...
        private void DrawSingleRule(SpawnRule rule, int spawnRuleID, bool helpEnabled)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            rule.m_name = m_editorUtils.TextField("RuleName", rule.m_name);
            if (rule.m_resourceType != SpawnerResourceType.StampDistribution)
            {
                bool allRP = rule.m_RP_BuiltIn && rule.m_RP_URP && rule.m_RP_HDRP;
                if (allRP)
                {
                    EditorGUI.BeginChangeCheck();
                    m_editorUtils.Label("AllRenderPipelines", GUILayout.Width(40));
                    allRP = EditorGUILayout.Toggle(allRP, GUILayout.Width(10));
                    if (EditorGUI.EndChangeCheck() && !allRP)
                    {
                        rule.m_RP_BuiltIn = false;
                        rule.m_RP_URP = false;
                        rule.m_RP_HDRP = false;
                    }
                }
                else
                {
                    m_editorUtils.Label("RPBuiltIn", GUILayout.Width(40));
                    rule.m_RP_BuiltIn = EditorGUILayout.Toggle(rule.m_RP_BuiltIn, GUILayout.Width(12));
                    m_editorUtils.Label("RPURP", GUILayout.Width(30));
                    rule.m_RP_URP = EditorGUILayout.Toggle(rule.m_RP_URP, GUILayout.Width(12));
                    m_editorUtils.Label("RPHDRP", GUILayout.Width(40));
                    rule.m_RP_HDRP = EditorGUILayout.Toggle(rule.m_RP_HDRP, GUILayout.Width(12));
                }
            }

            EditorGUILayout.EndHorizontal();
            m_editorUtils.InlineHelp("RuleName", helpEnabled);
            m_editorUtils.InlineHelp("AllRenderPipelines", helpEnabled);
            EditorGUI.indentLevel++;
            ...
            switch (sr.m_resourceType)
            {
                ...
                case SpawnerResourceType.TerrainDetail:
                    ...
                    if (oldTexture != null || oldGameObject != null)
                    {
                        ...
                        {
                            if (m_spawner.m_settings.m_resources.m_detailPrototypes[sr.m_resourceIdx].m_useInstancing || m_spawner.m_settings.m_resources.m_detailPrototypes[sr.m_resourceIdx].m_renderMode == DetailRenderMode.VertexLit || (m_spawner.m_settings.m_resources.m_detailPrototypes[sr.m_resourceIdx].m_renderMode == DetailRenderMode.Grass && oldGameObject != null))
                            {
        ...
        private void DrawSaveAndLoad(bool obj)
        {
            ...
            if (m_editorUtils.Button("SaveButton"))
            {
                ...
                string filename = GaiaUtils.GetSafeFilename(m_spawner.transform.name);
