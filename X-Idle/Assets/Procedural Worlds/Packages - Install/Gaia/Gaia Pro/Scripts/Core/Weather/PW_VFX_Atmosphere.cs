    ...
    public class PW_VFX_Atmosphere : MonoBehaviour
    {
                ...
                if (WeatherSystem.DoesAtmosphereNeedUpdate)
                {
                    if (WeatherSystem.m_renderPipeline != GaiaConstants.EnvironmentRenderer.HighDefinition)
                    {
                        if (m_gradientOrCurvesExists)
                        {
                            ...
                            if (!m_underwaterExists || !GaiaUnderwaterEffects.Instance.IsUnderwater)
                            {
                                //Fog Color
                                RenderSettings.fogColor = TimeOfDayFogColor.Evaluate(m_currentTime);
                                //Apply Extra Color
                                RenderSettings.fogColor = ProceduralWorldsGlobalWeather.UpdateAdditionalFogColor(RenderSettings.fogColor, AdditionalFogColor);
                            ...
                            if (CloudLayerMaterials.Count > 0)
                            {
                                Shader.SetGlobalColor(GaiaShaderID.m_cloudDomeFogColor, RenderSettings.fogColor);
                                ...
                                Shader.SetGlobalColor(GaiaShaderID.m_cloudDomeFinalSkyColor, TimeOfDaySkyColor.Evaluate(m_currentTime) * RenderSettings.fogColor);
                    ...
                    else
                    {
                        ...
                        if (m_gradientOrCurvesExists)
                        {
                            if (HDRPLightData != null)
                            {
                                ...
                                GaiaHDRPRuntimeUtils.SetLightIntensity(SunLight, HDRPLightData, SunBrightnessIntensity.Evaluate(m_currentTime) * GaiaConstants.HDRPSunIntensityMultiplier);
        ...
        private bool SetSunLight(bool isNight)
        {
            ...
            if (m_weatherManagerExists)
            {
                ...
                if (HDRPLightData != null)
                {
                    GaiaUtils.CheckHDRPLightRenderMode(SunLight, HDRPLightData, GaiaHDRPRuntimeUtils.GetLightIntensity(SunLight,HDRPLightData));
                }
#endif
                if (SunLight != null)
                {
        ...
        public void UpdateSystem()
        {
            ...
            if (WeatherSystem.m_renderPipeline == GaiaConstants.EnvironmentRenderer.BuiltIn || WeatherSystem.m_renderPipeline == GaiaConstants.EnvironmentRenderer.Universal)
            {
                ...
                if (m_gradientOrCurvesExists)
                {
                    ...
                    //Only adjust the fog if we are above sea level, or if underwater fog changes are allowed
                    if (ProceduralWorldsGlobalWeather.Instance.m_player.transform.position.y > GaiaAPI.GetSeaLevel() || FogUnderwater)
                    {
                ...
                if (m_skyboxMaterialExists)
                {
                    if (m_gradientOrCurvesExists)
                    {
                        if (GaiaUtils.ValidateShaderProperty(SkyboxMaterial, GaiaShaderID.m_unitySkyboxGroundColor))
                        {
                            SkyboxMaterial.SetColor(GaiaShaderID.m_unitySkyboxGroundColor, RenderSettings.fogColor);
                        }
                        if (GaiaUtils.ValidateShaderProperty(SkyboxMaterial, GaiaShaderID.m_unitySkyboxAtmosphereThickness))
                        {
                            SkyboxMaterial.SetFloat(GaiaShaderID.m_unitySkyboxAtmosphereThickness, AtmosphereThickness.Evaluate(m_currentTime));
