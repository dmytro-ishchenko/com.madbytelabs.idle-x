...
namespace Gaia
{
    ...
    public class ProceduralWorldsGlobalWeather : MonoBehaviour
    {
        ...
        {
            get { return m_additionalFogColor; }
            set
            {
                if (m_additionalFogColor.r != value.r || m_additionalFogColor.g != value.g || m_additionalFogColor.b != value.b)
                {
                    m_additionalFogColor = value;
                    if (AtmosphereSystemValid)
                    {
                        PW_VFX_Atmosphere.Instance.AdditionalFogColor = value;
                        PW_VFX_Atmosphere.Instance.UpdateSystem();
                    }
                }
            }
        }
        [SerializeField] private Color m_additionalFogColor = Color.black;
        ...
        public bool TODFogUnderwater
        {
            get { return m_todFogUnderwater; }
            set
            {
                if (m_todFogUnderwater != value)
                {
                    m_todFogUnderwater = value;
                    m_updateClouds = true;
                    UpdateWeatherEvents();
                }
            }
        ...
        [SerializeField] 
        private bool m_todFogUnderwater = false;
        [SerializeField]
        ...
        [SerializeField] private SceneProfile m_sceneProfile;
        ...
        {
            m_currentTime = PWSkyStandalone.GetTimeOfDayMainValue();
            SetAutoExposure();
        ...
        public void Initialize()
        {
            try
            {
                ...
                {
                    m_thunderComponent = FindAnyObjectByType<ThunderStrike>();
                }
                ...
                {
                    GaiaGlobal.Instance = FindAnyObjectByType<GaiaGlobal>();
                }
                ...
                if (PW_VFX_Atmosphere.Instance != null && CloudDomeBrightness != null)
                {
        ...
        private void UpdateSeasonMode()
        {
            try
            {
                if (EnableSeasons)
                {
                    ...
                    {
                        m_sceneProfile.m_gaiaWeather.m_season = Season;
                    }
                }
                else
                {
            ...
            try
            {
                if (Terrain.activeTerrains.Length > 0)
                {
                    foreach (Terrain terrain in Terrain.activeTerrains)
                    {
                        ...
                        terrain.terrainData.wavingGrassStrength = Mathf.Clamp(WindSpeed * 0.75f, MinTerrainWind, MaxTerrainWind);
                        //"Size" (Wind Wave Size) in  terrain inspector
                        terrain.terrainData.wavingGrassSpeed = Mathf.Clamp(WindSpeed * 0.6f, MinTerrainWind, MaxTerrainWind);
                        //"Bending" in terrain inspector
                        terrain.terrainData.wavingGrassAmount = Mathf.Clamp(WindSpeed / 3.5f, MinTerrainWind, MaxTerrainWind);
        ...
        private void UpdateCloudSettings()
        {
            try
            {
                ...
                if (AtmosphereSystemValid)
                {
                    ...
                    PW_VFX_Atmosphere.Instance.FogUnderwater = TODFogUnderwater;
        ...
        private bool EnableSnowSystem()
        {
            try
            {
                ...
                if (!m_snowAudioSource.isPlaying && m_snowAudioSource.isActiveAndEnabled)
                {
        ...
        {
            if (FindAnyObjectByType<CompleteTerrainShader>() == null)
            {
                return null;
            }

            CTSWeatherManager cTSWeatherManager = FindAnyObjectByType<CTSWeatherManager>();
            ...
            try
            {
                ...
                if (m_renderPipeline == GaiaConstants.EnvironmentRenderer.HighDefinition)
                {
                    ...
                    if (lightData != null)
                    {
                        ...
                        {
                            GaiaHDRPRuntimeUtils.SetLightIntensity(currentLightSource, lightData, weatherSettings.m_sunIntensity.Evaluate(m_currentTime) * GaiaConstants.HDRPSunIntensityMultiplier);
                        }
                ...
                else
                {
                    ...
                    if (GaiaUnderwaterEffects.Instance == null || !GaiaUnderwaterEffects.Instance.IsUnderwater)
                    {
                        if (weatherSettings.m_fogColor != null)
                        {
                            ...
                            //Apply Extra Color
                            RenderSettings.fogColor = UpdateAdditionalFogColor(RenderSettings.fogColor, AdditionalFogColor);
                        }
                        ...
                        if (AtmosphereSystemValid)
                        {
                            ...
                            {
                                RenderSettings.fogDensity = weatherSettings.m_fogDensity.Evaluate(m_currentTime) + AdditionalFogDistanceExponential;
                            }
                            if (weatherSettings.m_fogEndDistance != null)
                            {
                                float additionalFogValue = AdditionalFogDistanceLinear;
                                if (additionalFogValue < 0)
                                {
                                    RenderSettings.fogEndDistance = weatherSettings.m_fogEndDistance.Evaluate(m_currentTime) + Mathf.Abs(AdditionalFogDistanceLinear);
                                }
                                else
                                {
                                    RenderSettings.fogEndDistance = weatherSettings.m_fogEndDistance.Evaluate(m_currentTime) - AdditionalFogDistanceLinear;
                                    if (RenderSettings.fogEndDistance < RenderSettings.fogStartDistance + 0.1f)
                                    {
                                        RenderSettings.fogEndDistance = RenderSettings.fogStartDistance + 0.1f;
                                    }
                                }
                            }
                        }
            ...
            try
            {
                ...
                if (lightData != null)
                {
                    ...
                    if (enabling)
                    {
                        if (weatherSettings.m_sunIntensity != null)
                        {
                            GaiaHDRPRuntimeUtils.SetLightIntensity(currentLightSource,lightData, Mathf.Lerp(CurrentSettings.m_sunIntensity, weatherSettings.m_sunIntensity.Evaluate(m_currentTime) * GaiaConstants.HDRPSunIntensityMultiplier, GetFadeTime()));
                        }
                        if (weatherSettings.m_sunColor != null)
                        {
            ...
            try
            {
                if (enabling)
                {
                    ...
                    if (AtmosphereSystemValid)
                    {
                        if (weatherSettings.m_fogDensity != null)
                        {
                            RenderSettings.fogDensity = Mathf.Lerp(CurrentSettings.m_fogDensity, weatherSettings.m_fogDensity.Evaluate(m_currentTime) + AdditionalFogDistanceExponential, GetFadeTime());
                        }
                        if (weatherSettings.m_fogEndDistance != null)
                        {
                            float additionalFogValue = AdditionalFogDistanceLinear;
                            if (additionalFogValue < 0)
                            {
                                RenderSettings.fogEndDistance = Mathf.Lerp(CurrentSettings.m_fogEnd, weatherSettings.m_fogEndDistance.Evaluate(m_currentTime) + Mathf.Abs(AdditionalFogDistanceLinear), GetFadeTime());
                            }
                            else
                            {
                                RenderSettings.fogEndDistance = Mathf.Lerp(CurrentSettings.m_fogEnd, weatherSettings.m_fogEndDistance.Evaluate(m_currentTime) - AdditionalFogDistanceLinear, GetFadeTime());
                                if (RenderSettings.fogEndDistance < RenderSettings.fogStartDistance + 0.1f)
                                {
                                    RenderSettings.fogEndDistance = RenderSettings.fogStartDistance + 0.1f;
                                }
                ...
                else
                {
                    ...
                    if (AtmosphereSystemValid)
                    {
                        RenderSettings.fogDensity = Mathf.Lerp(CurrentSettings.m_fogDensity, PW_VFX_Atmosphere.Instance.FogDensityColor.Evaluate(m_currentTime) + AdditionalFogDistanceExponential, GetFadeTime());
                        float additionalFogValue = AdditionalFogDistanceLinear;
                        if (additionalFogValue < 0)
                        {
                            RenderSettings.fogEndDistance = Mathf.Lerp(CurrentSettings.m_fogEnd, PW_VFX_Atmosphere.Instance.FogEndDistance.Evaluate(m_currentTime) + Mathf.Abs(AdditionalFogDistanceLinear), GetFadeTime());
                        }
                        else
                        {
                            RenderSettings.fogEndDistance = Mathf.Lerp(CurrentSettings.m_fogEnd, PW_VFX_Atmosphere.Instance.FogEndDistance.Evaluate(m_currentTime) - AdditionalFogDistanceLinear, GetFadeTime());
                            if (RenderSettings.fogEndDistance < RenderSettings.fogStartDistance + 0.1f)
                            {
                                RenderSettings.fogEndDistance = RenderSettings.fogStartDistance + 0.1f;
                            }
                        ...
                        if (m_hdrpVolumeProfile.TryGet(out physicallyBasedSky))
                        {
                            if (m_hdrpVolumeProfile.TryGet(out hDRISky))
                            {
                                if (m_hdrpVolumeProfile.TryGet(out gradientSky))
                                {
                                    ...
                                    if (setAllDefaults)
                                    {
                                        //there does not seem to be a counterpart for these settings in the newer HDRP versions
        ...
        private WindZone GetOrCreateWindZone()
        {
            try
            {
                ...
                WindZone windSettings = FindAnyObjectByType<WindZone>();
                ...
                if (windZone == null)
                {
                    ...
                    windZone.transform.parent = transform.parent.parent;
#endif
        ...
        public PostProcessProfile GetPostProcessingProfile()
        {
            try
            {
                m_volumes = FindObjectsByType<PostProcessVolume>(FindObjectsSortMode.None);
        ...
        public void SetPostProcessingProfile(PostProcessProfile profile, bool directToCamera)
        {
            try
            {
                ...
                PostProcessLayer processLayer = FindAnyObjectByType<PostProcessLayer>();
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (HDLightData != null)
                    {
                        ...
                        CurrentSettings.m_sunIntensity = GaiaHDRPRuntimeUtils.GetLightIntensity(currentLightSource, HDLightData);
                    }
#endif
                }
        ...
        /// <summary>
        /// Sets up the extra fog color in RGB channels
        /// </summary>
        /// <param name="currentColor"></param>
        /// <param name="extraR"></param>
        /// <param name="extraG"></param>
        /// <param name="extraB"></param>
        /// <returns></returns>
        public static Color UpdateAdditionalFogColor(Color currentColor, Color extraFogColor)
        {
            Color newFogColor = currentColor;
            //Red
            if (extraFogColor.r >= 0)
            {
                Mathf.Clamp01(newFogColor.r += extraFogColor.r);
            }
            else
            {
                Mathf.Clamp01(newFogColor.r -= Mathf.Abs(extraFogColor.r));
            }
            //Green
            if (extraFogColor.g >= 0)
            {
                Mathf.Clamp01(newFogColor.g += extraFogColor.g);
            }
            else
            {
                Mathf.Clamp01(newFogColor.g -= Mathf.Abs(extraFogColor.g));
            }
            //Blue
            if (extraFogColor.b >= 0)
            {
                Mathf.Clamp01(newFogColor.b += extraFogColor.b);
            }
            else
            {
                Mathf.Clamp01(newFogColor.b -= Mathf.Abs(extraFogColor.b));
            }
            //Apply
            return newFogColor;
        }
        ...
        public static void RemoveGlobalWindShader()
        {
            ...
            PWSkyPostProcessingManager weatherPostFX = FindAnyObjectByType<PWSkyPostProcessingManager>();
            if (weatherPostFX != null)
            {
                Object.DestroyImmediate(weatherPostFX);
            }
        }
        /// <summary>
        ...
        public static Transform GetPlayer()
        {
            try
            {
                ...
                m_mainCameras = FindObjectsByType<Camera>(FindObjectsSortMode.None);
                if (m_mainCameras.Length > 0)
                {
                    foreach (Camera camera in m_mainCameras)
                    {
                        ...
                        if (GaiaUtils.DoesTagExist("Camera"))
                        {
                            if (camera.CompareTag("Camera"))
                            {
                                player = camera.transform;
                                break;
                            }
