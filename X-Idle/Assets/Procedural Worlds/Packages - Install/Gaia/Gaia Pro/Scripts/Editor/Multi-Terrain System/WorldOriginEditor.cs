...
namespace Gaia
{
    ...
    class WorldOriginEditor
    {
        ...
        private static bool worldDesignerChecked;

        private static GameObject WorldDesigner
        {
            get
            {
                if (m_worldDesigner == null && !worldDesignerChecked)
                {
                    m_worldDesigner = GaiaUtils.GetOrCreateWorldDesigner(false, false);
   
                    //world designer was null when we looked for it? Susbcribe to the creation event to figure out when it is "worth it" to check again
                    if (m_worldDesigner == null)
                    {
                        worldDesignerChecked = true;
                        Gaia.WorldDesigner.OnWorldDesignerCreated -= OnWorldDesignerCreated;
                        Gaia.WorldDesigner.OnWorldDesignerCreated += OnWorldDesignerCreated;
                    }
                }
        ...
        private static void OnWorldDesignerCreated(GameObject worldDesigner)
        {
            m_worldDesigner = worldDesigner;
            worldDesignerChecked = false;
            Gaia.WorldDesigner.OnWorldDesignerCreated -= OnWorldDesignerCreated;
        }
        ...
        public static void RenderSceneGUI(SceneView sceneview)
        {
            ...
            //    }
            //}
            //Now we can draw the panel last so it draws on top of all the gizmos etc.
            DrawTopPanel();
            Handles.EndGUI();
