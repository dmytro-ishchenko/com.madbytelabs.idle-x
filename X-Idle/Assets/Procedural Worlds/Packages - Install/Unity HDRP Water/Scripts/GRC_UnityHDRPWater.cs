...
namespace Gaia
{
    public class GRC_UnityHDRPWater : GaiaRuntimeComponent
    {
...
#if HDPipeline && UNITY_2022_3_OR_NEWER
                WaterSurface ws = newWaterGO.GetComponent<WaterSurface>();
                if (ws != null)
                {
                    ws.foam = m_activateFoam;
                }
#endif
            }
        ...
        private void ConfigureHDRPWater()
        {
            ...
            //this has been made difficult to access in Unity 6, so we can only do this in an automated fashion in earlier versions.
#if UNITY_6000_0_OR_NEWER
            if (!UnityEditor.EditorUtility.GetDialogOptOutDecision(DialogOptOutDecisionType.ForThisMachine, "GaiaDoNotShowWaterInfo"))
            {
                int i = EditorUtility.DisplayDialogComplex("Please enable HDRP Water settings", "Water needs to be enabled in your HD render pipeline asset.\r\n\r\nIf you do not see a water surface, please select the 'Gaia Runtime > HDRP Water' object in your scene. It should offer a button that will take you to the water settings in the render pipeline asset.\r\n\r\nAdditionally, please review the section 'Use the water system in your project' in the HDRP manual to make sure you have water enabled in all the required places.", "OK", "Do not show this again.", "Open HDRP Manual");

                if (i == 1)
                {
                    UnityEditor.EditorUtility.SetDialogOptOutDecision(UnityEditor.DialogOptOutDecisionType.ForThisMachine, "GaiaDoNotShowWaterInfo", true);
                    return;
                }
                if (i == 2)
                {
                    Application.OpenURL("https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.4/manual/water-use-the-water-system-in-your-project.html");
                }
            }
#else
