...
namespace ProceduralWorlds.GTS
{
    ...
    ///// </summary>
    public enum ProgressBarPriority { TerrainLoading, MinMaxHeightCalculation, CreateBiomeTools, CreateSceneTools, BiomeRemoval, MultiTerrainAction, TerrainMeshExport, MaskMapExport, WorldCreation, Stamping, Spawning, GaiaGeospatial, Maintenance };
    ...
    public static class ProgressBar
    {
        ...
        public static bool Show(ProgressBarPriority priority, string title, string info, int currentStep = 0, int totalSteps = 0, bool displayETA = false, bool cancelable = false)
        {
            ...
            {
                m_averageStepDuration = (m_averageStepDuration * (currentStep -1) + (GTSUtility.GetUnixTimestamp() - m_lastStepTimeStamp)) / (long)currentStep;
            }
            m_lastStepTimeStamp = GTSUtility.GetUnixTimestamp();
